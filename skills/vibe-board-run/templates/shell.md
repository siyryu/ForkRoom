...
update_vibe_run() {
  ...
  RUN_FILE="$run_file" RUN_STATUS="$status" RUN_PROGRESS="$progress" RUN_MESSAGE="$message" RUN_ETA="$estimated_end_at" RUN_UPDATED_AT="$updated_at" python3 - <<'PY'
import json
import os
...
progress = int(os.environ["RUN_PROGRESS"]) if os.environ["RUN_PROGRESS"] else run.get("progress")
...
```bash
update_vibe_run ".agents/exps/<exp-id>/runs/<run-id>.json" running 45 "Processed 45%" "2026-06-22T18:30:00+08:00"
```
